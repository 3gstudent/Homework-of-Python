...
def find_all_people(host, mode, user, data, QueryString):
...
def get_size_of_folder(host, mode, user, data, folder):
    POST_BODY = '''<?xml version="1.0" encoding="utf-8"?>
...
def list_mail_of_folder(host, mode, user, data, folder, offset, size):
    POST_BODY = '''<?xml version="1.0" encoding="utf-8"?>
<soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:m="http://schemas.microsoft.com/exchange/services/2006/messages" xmlns:t="http://schemas.microsoft.com/exchange/services/2006/types" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
  <soap:Header>
    <t:RequestServerVersion Version="Exchange2013_SP1" />
...
def save_mail(host, mode, user, data, Id, ChangeKey, savepath):
    POST_BODY = '''<?xml version="1.0" encoding="utf-8"?>
<soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:m="http://schemas.microsoft.com/exchange/services/2006/messages" xmlns:t="http://schemas.microsoft.com/exchange/services/2006/types" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
  <soap:Header>
    <t:RequestServerVersion Version="Exchange2013_SP1" />
...
def get_attachment(host, mode, user, data, ItemId, folderpath):
    POST_BODY = '''<?xml version="1.0" encoding="utf-8"?>
<soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:m="http://schemas.microsoft.com/exchange/services/2006/messages" xmlns:t="http://schemas.microsoft.com/exchange/services/2006/types" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
  <soap:Header>
    <t:RequestServerVersion Version="Exchange2013_SP1" />
...
'''
    ...
    if res.status_code == 200 and "NoError" in res.text:
        ...
        name = pattern_name.findall(res.text)
        for i in range(len(name)):
            save_attachment(host, mode, user, data, name[i], folderpath)
    else: 
        print(res.status_code)  
        print(res.text)
        sys.exit(0)
...
def save_attachment(host, mode, user, data, AttachmentId, folderpath):
    POST_BODY = '''<?xml version="1.0" encoding="utf-8"?>
<soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:m="http://schemas.microsoft.com/exchange/services/2006/messages" xmlns:t="http://schemas.microsoft.com/exchange/services/2006/types" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
  <soap:Header>
    <t:RequestServerVersion Version="Exchange2013_SP1" />
...
def search_mail(host, mode, user, data, querystring, folderpath, currentpath):
    POST_BODY = '''<?xml version="1.0" encoding="utf-8"?>
<soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:m="http://schemas.microsoft.com/exchange/services/2006/messages" xmlns:t="http://schemas.microsoft.com/exchange/services/2006/types" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
  <soap:Header>
    <t:RequestServerVersion Version="Exchange2013_SP1" />
...
'''
    ...
    if res.status_code == 200 and "NoError" in res.text:
        ...
        for i in range(len(name)): 
            ...
            save_mail(host, mode, user, data, Id, ChangeKey, currentpath+'\\'+user+'\\Search-'+escape2(querystring)+'\\')
            if "HasAttachments>true" in name[i]:
                print('\n[+] %s'%(Id))
                get_attachment(host, mode, user, data, Id, 'Search-'+escape2(querystring))
    else: 
        print(res.status_code)  
        print(res.text)
        sys.exit(0)
...
if __name__ == '__main__':
    if len(sys.argv)!=6:    
        print('ewsManage_requests_ntlm_Downloader.py')       
        ...
        if sys.argv[5] == "findallpeople":
            ...
            for i in range(97,123):
                find_all_people(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], chr(i))
            ...
            if folderpath == "inboxall":
                size = get_size_of_folder(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], "inbox")
                print("[+] inbox size: " + size)
                name = list_mail_of_folder(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], "inbox", 0, size)
               
            elif folderpath == "sentitemsall":
                size = get_size_of_folder(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], "sentitems")
                print("[+] sentitems size: " + size)
                name = list_mail_of_folder(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], "sentitems", 0, size)

            else:
                size = get_size_of_folder(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], folderpath)
                print("[+] %s size: %s"%(folderpath, size))
                offset = input("Input the start position(0):")
                size = input("Input the size:")                
                name = list_mail_of_folder(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], folderpath, offset, size)
            ...
            for i in range(len(name)): 
                ...
                strlist2 = Temp.split('\"/>')
                ChangeKey = strlist2[0]
                save_mail(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], Id, ChangeKey, path2+"\\")
                if "HasAttachments>true" in name[i]:
                    print('\n[+] %s'%(Id))
                    get_attachment(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], Id, folderpath)
            ...
            search_mail(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], querystring, "inbox", path1)
            search_mail(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], querystring, "sentitems", path1)            
